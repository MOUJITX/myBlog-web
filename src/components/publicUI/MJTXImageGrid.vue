...
import {Delete, Edit, Link, Search, Select, CloseBold} from "@element-plus/icons-vue";
...
import {defaultFile, deleteFileAPI, getImagesPage, renameFileAPI} from "@/api/file";
...
const getDataList = () => {
  getImagesPage(listQuery.value.currentPage,listQuery.value.pagesize,searchForm.value).then(res => {
    if (!res) return;
...
const selectFile = (download_url: string) => {
  ...
  if (props.limit && selectImages.value.length >= props.limit) {
    ElNotification.warning({title: '图片选择失败', message: '最多只能选择' + props.limit + '个文件'})
    return;
  }
...
const copyToClipboard = async (text: string) => {
  try {
    ...
    ElNotification.error({title: '图片地址复制失败，当前浏览器似乎不支持复制功能'})
  }
};
...
const renameImage = (id: string, originName: string, index: number) => {
  if (!imageName.value[index]) { 
    ElNotification.warning({ title: "重命名失败", message: "文件名不允许为空"})
    return;
  } else if (imageName.value[index].length > 255) { 
    ElNotification.warning({ title: "重命名失败", message: "文件名长度不允许大于255个字符"})
    return;
  }
...
<template>
  <MJTXInline class="mjtx-imageGrid-search">
    ...
    <el-button @click="getDataList"><el-icon><Search /></el-icon></el-button>
  </MJTXInline>
  <div class="mjtx-imageGrid">
    <div class="mjtx-imageGrid-image"
      ...
      <span class="mjtx-imageGrid-name">{{ image.file_name+image.end_name }}</span>
      <span class="mjtx-imageGrid-btn">
        <el-icon @click="copyToClipboard(image.download_url)"><Link /></el-icon>
        <el-popover placement="bottom-start" title="修改图片标题" trigger="click" width="300" ref="refRenamePopover" :visible="visible[index]">
          <MJTXInline position="center">
            <el-input v-model="imageName[index]"></el-input>
            <el-button type="primary" :icon="Select" circle size="small" @click="renameImage(image.uuid, image.file_name, index)"/>
            <el-button :icon="CloseBold" circle size="small" @click="visible[index] = false" />
          </MJTXInline>
          <template #footer></template>
          <template #reference>
            <el-icon @click="openRenamePopover(image.file_name, index)"><Edit /></el-icon>
          </template>
        </el-popover>
      ...
      <div class="mjtx-imageGrid-mask" v-if="selectImages.includes(image.download_url)">
        {{selectImages.indexOf(image.download_url)+1}}
      </div>
      <div :class="`mjtx-imageGrid-select mjtx-imageGrid-select__${selectImages.includes(image.download_url) ? 'on' : 'off'}`"
