...
<template>
  <el-button :type="props.type" @click="formVisible = true" v-if="popupForm.includes(props.form)" :link="props.link" :disabled="props.disabled" :size="props.buttonSize">
    {{ props.buttonTitle }}
  </el-button>
  <router-link :to="props.routerPath" :target="props.form === 'page' ? '_self' : '_blank'" v-else>
    <el-button :type="props.type" class="formButton" :link="props.link" :disabled="props.disabled" :size="props.buttonSize">{{ props.buttonTitle }}</el-button>
  </router-link>
  <el-dialog
      append-to-body
      class="form-popup"
