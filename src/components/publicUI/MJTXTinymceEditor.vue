<template>
  <editor
    v-model="textContent"
    :init="init"
    :tinymceScriptSrc="tinymceScriptSrc"></editor>
</template>
...
<script setup lang="ts">
  ...
  const plugins =
    'axupimgs advlist anchor autolink autosave ' + //autoresize
    ' ' + //bdmap bbcode
  ...
  const fontSize =
    '12px 14px 16px 18px 20px 22px 24px 28px 32px 36px 48px 56px 72px 80px 90px 100px 110px';

  const fontFormat =
    '方正小标宋简体=方正小标宋简体; 苹果苹方=PingFang SC, 宋体=宋体,SimSun; 黑体=黑体,SimHei; ' +
  ...
  const init = {
    ...
    axImages_upload_handler: function (
      blobInfo: any,
      resolve: any,
      reject: any,
    ) {
