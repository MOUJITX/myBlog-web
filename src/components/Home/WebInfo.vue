...
const profile_concat:IWebsiteConcat[] = JSON.parse(localStorage.getItem('profile_concat') || '[]')
</script>

<template>
  <MJTXCard :z-index="100">
    <div class="web-top">
      <div class="web-user" :style="{
  'background-image': `linear-gradient(to bottom, rgba(255,255,255,0), rgba(255,255,255,1) 70%), url(${imgToThumb(profile_background as string,640)})`,
    ...
    <div class="web-bottom">
      <a class="web-contact-icon" v-for="(item,key) in profile_concat" :key="key" :href="item.cc_url" target="_blank" :title="item.cc_name">
        <img :src="imgToThumb(item.cc_icon, 64)" :alt="item.cc_name"/>
        <div class="web-contact-icon-qrcode" v-if="item.cc_value">{{ item.cc_value }}</div>
      </a>
    </div>
  </MJTXCard>
</template>
