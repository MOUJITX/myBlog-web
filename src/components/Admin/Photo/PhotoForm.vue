...
import {defaultPhoto, defaultPhotoImage, insetPhoto, selectPhotoByID, updatePhoto} from "@/api/photo";
...
const editImageGroup = (targetIndex: number, action: 'remove' | 'add') => {
  ...
  if (action === 'add') {
    ...
    photoForm.value.images.splice(editGroupValue.value,0,{...defaultPhotoImage})
    photoForm.value.images[editGroupValue.value].images = []
  } else if (action === 'remove') {
    if (targetIndex === editGroupValue.value) editGroupValue.value = targetIndex - 1
    else if (targetIndex < editGroupValue.value) editGroupValue.value = editGroupValue.value - 1
...
<template>
  <el-form ref="formRef" :model="photoForm" :rules="rules" status-icon label-position="top">
    ...
    <el-form-item label="图片" prop="images">
      <el-tabs type="border-card" style="width: 100%" editable @edit="editImageGroup" v-model="editGroupValue">
        <span v-if="photoForm.images.length === 0">暂无照片组</span>
        <el-tab-pane
            v-else
            v-for="(group,index) in photoForm.images" :key="index"
