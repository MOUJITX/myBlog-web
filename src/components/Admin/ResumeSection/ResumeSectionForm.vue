...
const rules = reactive<FormRules<IResumeSection>>({
  ...
  description:[
    {required: false, message:'不允许为空', trigger: 'blur'}
  ]
})
...
<template>
  <el-form ref="formRef" :model="resumeSectionForm" :rules="rules" label-width="auto" status-icon>
    ...
    <el-form-item label="宽度%" prop="min_width">
      <el-input-number v-model="resumeSectionForm.min_width" min="0" max="100"/>
      -
      <el-input-number v-model="resumeSectionForm.max_width" min="0" max="100"/>
    </el-form-item>
    <el-form-item label=" ">
