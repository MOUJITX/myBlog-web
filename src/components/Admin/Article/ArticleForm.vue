...
<template>
  <el-form ref="formRef" :model="articleForm" :rules="rules" label-width="auto" status-icon>
    <el-form-item label="标题" prop="title">
      <el-input v-model="articleForm!.title">
        <template #append>
          <el-check-tag type="danger" :checked="articleForm!.is_top" @change="articleForm!.is_top = !articleForm!.is_top">
    ...
    <el-form-item label="描述" prop="description">
      <template #label>
        <div>描述</div>
        <div @click="refreshDescription(articleForm?.full_content?.toString())" ><el-icon><Refresh /></el-icon></div>
      </template>
    ...
    <el-form-item label="来源" prop="source_url">
      <el-input v-model="articleForm!.source_url">
        <template #append>
          <el-check-tag type="primary" :checked="articleForm!.is_link" @change="articleForm!.is_link = !articleForm!.is_link">
    ...
    <el-form-item label="密码访问" prop="view_code">
      <div class="articleSetting-viewCode">
        ...
        <el-input v-model="articleForm!.view_code" v-if="articleForm!.is_private" />
      </div>
    </el-form-item>
    <el-form-item label="标签" prop="tags" >
      <el-select v-model="articleForm!.tags" multiple filterable >
        <el-option v-for="item in tags" :key="item.uuid" :label="item.tag" :value="item.uuid"/>
      </el-select>
