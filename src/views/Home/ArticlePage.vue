...
<template>
  <BannerBox type="custom" :title="articleData.title!" :banner-image="articleData.image_url" blur>
    <MJTXInline>
      <router-link v-for="(item,index) in articleData.categories_name" :key="index" :to="`/list/category/${articleData.categories?.[index]}`">
        <MJTXTag :tag="item" size="14px" round/>
      </router-link>
      <router-link v-for="(item,index) in articleData.tags_name" :key="index" :to="`/list/tag/${articleData.tags?.[index]}`">
    ...
    <MJTXInline position="center">
      ...
      <MJTXTag tag="原创" size="14px" :round="false" v-if="articleData.is_original"/>
      ...
      <span class="article-banner_tip">{{ articleData.create_time!.substring(0,10) }}</span>
    </MJTXInline>
  </BannerBox>
