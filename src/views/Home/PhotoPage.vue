...
<template>
  <BannerBox type='page' :title="photoData.title" :sub-title="photoData.subtitle" :banner-image="photoData.index_img"/>
  <div class="photo">
    <div v-for="(photo, index) in photoData.images" :key="index">
      <MJTXTextLine v-if="photo.title" :text="photo.title" :tip="photo.subtitle" size="26px" :description="photo.description"/>
      <div class="photo-list">
        <PhotoShow v-for="(images, index) in photo.images" :key="index" :photo="images" :is-menu="false"/>
      </div>
    </div>
  </div>
</template>
