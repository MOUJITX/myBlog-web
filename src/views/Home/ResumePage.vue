...
const pageSettingItem: IWebsiteNavigation = JSON.parse(localStorage.getItem('web_navigate') || '[]').find((item: { nav_url: string; }) => item.nav_url === '/resume')
...
<template>
  ...
  <div class="resume">
    <div v-for="(resume,index) in resumes" :key="index" :style="{'min-width': resume.min_width + '%','max-width': resume.max_width + '%'}">
      <MJTXTextLine :text="resume.section" size="26px"/>
      <div :class="resume.isrow ? 'resume-skill-apps' : ''">
        <InfoCard v-for="(item,resumeIndex) in resume.resumes" :key="resumeIndex"
                  :icon="item.icon.icon"
