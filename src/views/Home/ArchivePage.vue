...
const getDataList = async () => {
  topDataList.value = []
  getArticlesList(listQuery.value.currentPage,listQuery.value.pagesize,searchForm.value).then(res => {
    if (!res) return;
...
const pageSettingItem: IWebsiteNavigation = JSON.parse(localStorage.getItem('web_navigate') || '[]').find((item: { nav_url: string; }) => item.nav_url === '/archive')
...
<template>
  <BannerBox :title="pageSettingItem.nav_title" :sub-title="pageSettingItem.nav_subtitle" :banner-image="pageSettingItem.nav_banner[rand]" type="page" />
  <div class="archive">
    <div class="archive-list" v-if="topDataList.length > 0">
      <MJTXRouterLink class="archive-article"
                      v-for="(article,index) in topDataList"
                      :key="index"
                      :to="article.is_link ? article.source_url! : `/article/${article.uuid}`" >
        <div class="archive-title archive-title_top">{{article.title}}</div>
        <div class="archive-date" >{{dateFormat(article.create_time,"YYYY/MM/DD")}}</div>
      </MJTXRouterLink>
    </div>
    <div class="archive-list" v-for="(yearItem,index) in dataList" :key="index">
