...
const pageSettingItem: IWebsiteNavigation = JSON.parse(localStorage.getItem('web_navigate') || '[]').find((item: { nav_url: string; }) => item.nav_url === '/photo')
...
const getDataList = () => {
  getPhotoList(listQuery.value.currentPage,listQuery.value.pagesize).then(res => {
    if (!res) return;
...
<template>
  ...
  <div class="photo">
    <div class="photo-list">
      <PhotoShow v-for="(photo, index) in listData" :key="index" :index="index" :title="photo.title" :uuid="photo.uuid" :photo="photo.index_img"/>
    </div>
    <MJTXPagination
