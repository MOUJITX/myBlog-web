...
const getDataList = () => {
  getTagsPage(listQuery.value.currentPage,listQuery.value.pagesize).then(res => {
    if (!res) return;
  ...
  <MJTXCard padding>
    <el-table ref="tableRef" :data="tableData" style="width: 100%" max-height="calc(100vh - 280px)" @selectionChange="tableRowSelect">
      ...
      <el-table-column fixed="right" label="操作">
        <template #default="scope">
          <TagPopup :uuid="scope.row.uuid" @popupClose="getDataList" action="update"/>
          <TagPopup :uuid="scope.row.uuid" @popupClose="getDataList" action="copy"/>
