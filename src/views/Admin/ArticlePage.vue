...
import {Comment, Flag, Link, Lock, QuestionFilled, Star} from "@element-plus/icons-vue";
import CategoryPopup from "@/components/Admin/Category/CategoryPopup.vue";
import {ElNotification} from "element-plus";
import {defaultArticle, deleteArticleAPI, deleteArticlesAPI, getArticlesPage} from "@/api/article";
...
const getDataList = () => {
  getArticlesPage(listQuery.value.currentPage, listQuery.value.pagesize, searchForm.value).then(res => {
    if (!res) return;
...
<template>
  <div class="articlePage-container">
        ...
        >
          <template #default="{ node, data }">
            <span class="articlePage-tree-button">
              ...
              <span>
                <CategoryPopup link action="insert" @popupClose="reload" :currentUUID="data.uuid"/>
                <el-popconfirm title="确认删除？" @confirm="deleteCategorySingle(data.uuid)">
                  <template #reference>
                    <el-button link type="danger" :disabled="data.hasChildren">删除</el-button>
                  </template>
                </el-popconfirm>
              </span>
            </span>
    ...
    <div class="article-table">
      <MJTXCard padding>
        <MJTXFormButton type="primary" form="tab" button-title="新增"
      ...
      <MJTXCard padding>
        <el-table ref="tableRef" :data="tableData" style="width: 100%" max-height="calc(100vh - 280px)" @selectionChange="tableRowSelect">
          ...
          <el-table-column prop="title" label="标题">
            <template #default="scope">
              <div class="article-table-title">
                <router-link :to="`/article/${scope.row.uuid}`" target="_blank">{{scope.row.title}}</router-link>
          ...
          <el-table-column prop="update_time" label="最后更新时间" width="170"/>
          <el-table-column fixed="right" label="操作" width="170">
            <template #default="scope">
              <MJTXFormButton type="primary" link form="tab" button-title="编辑"
...
<!--              <ArticlePopup v-model="scope.row" link button-size="default" button-title="属性" type="primary" @popupClose="editArticleInfo(scope.row)"/>-->
              <MJTXFormButton type="primary" link form="tab" button-title="复制"
