...
import {defaultResume, deleteResumeAPI, deleteResumesAPI, getResumesPage} from "@/api/resume";
...
const getDataList = () => {
  getResumesPage(listQuery.value.currentPage,listQuery.value.pagesize,searchForm.value).then(res => {
    if (!res) return;
...
<template>
  ...
  <MJTXCard padding>
    <el-table ref="tableRef" :data="tableData" style="width: 100%" max-height="calc(100vh - 280px)" @selectionChange="tableRowSelect" @filter-change="tableFilter">
      <el-table-column type="selection" width="55" />
      <el-table-column prop="section_name" label="板块" :filters="resumeSections" :filter-multiple="false" column-key="section">
      ...
      <el-table-column fixed="right" label="操作">
        <template #default="scope">
          <ResumePopup :uuid="scope.row.uuid" @popupClose="getDataList" action="update"/>
          <ResumePopup :uuid="scope.row.uuid" @popupClose="getDataList" action="copy"/>
