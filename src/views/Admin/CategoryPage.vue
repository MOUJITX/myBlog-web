...
import {deleteCategoriesAPI, deleteCategoryAPI, getCategories, getCategoriesPage} from "@/api/category";
...
const getDataList = () => {
  getCategoriesPage(listQuery.value.currentPage, listQuery.value.pagesize, currentUUID.value).then(res => {
    if (!res) return;
...
<template>
  <div class="categoryPage-container">
    ...
    <div class="categoryPage-table">
      <MJTXCard padding>
        <CategoryPopup action="insert" @popupClose="reload" :currentUUID="currentUUID" />
      ...
      <MJTXCard padding>
        <el-table ref="tableRef" :data="tableData" style="width: 100%" max-height="calc(100vh - 280px)" @selectionChange="tableRowSelect">
          ...
          <el-table-column prop="banner_image" label="配图">
            <template #default="scope">
              <MJTXImageUpload v-model="scope.row.banner_image" disabled v-if="scope.row.banner_image"/>
            </template>
          </el-table-column>
          ...
          <el-table-column fixed="right" label="操作">
            <template #default="scope">
              <CategoryPopup :uuid="scope.row.uuid" @popupClose="reload" action="update" :currentUUID="currentUUID" />
              <CategoryPopup :uuid="scope.row.uuid" @popupClose="reload" action="copy" :currentUUID="currentUUID" />
...
.categoryPage-container{
  ...
  .categoryPage-tree{ width: 20% }
  .categoryPage-table{ width: 80% }
}
</style>
