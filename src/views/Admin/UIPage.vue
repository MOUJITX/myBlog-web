...
<template>
  ...
  <MJTXCard padding>
    <div style="display: flex">
      <MJTXIcon size="48" backgroundColor="dodgerblue" text-color="white" icon="标" type="round"/>
      <MJTXIcon size="48" backgroundColor="dodgerblue" text-color="white" icon="api/" type="round" padding/>
    </div>
    ...
    <div>type:String('circle' | 'round' | 'square') 图标类型(圆形、圆角、方型)</div>
  ...
  <MJTXCard padding>
    <div style="display: flex">
      <InfoCard icon="图" description="描述文字" sub-title="子标题" title="标题" />

      <InfoCard icon="api/标" description="描述文字" sub-title="子标题" title="标题" direction="column" icon-padding/>
    </div>
  ...
  <MJTXCard padding>
    <MJTXPagination v-model:current-page="listQuery.currentPage" v-model:page-size="listQuery.pagesize" :total="total"/>
    <div>v-model:current-page 当前页</div>
    <div>v-model:page-size 页大小</div>
    <div>total:Number 总计</div>
    <div>layout:String 显示内容 total总计, sizes页大小调整, prev上一页, pager页面选择, next下一页</div>
  ...
  <MJTXCard padding>
    <MJTXTimeLine time="2024" time-size="large" event="事件主标题" description="描述内容"/>
    <MJTXTimeLine time="07/01" time-size="medium" description="12"><div>123</div></MJTXTimeLine>
  ...
  <MJTXCard padding>
    ...
    <MJTXFormButton type="text" form="page" button-title="当前页面" router-path="/admin/setting">当前页面打开</MJTXFormButton>
    <MJTXFormButton form="tab" button-title="新标签页" router-path="/">新标签页打开</MJTXFormButton>
    <div>type:String("primary" | "success" | "warning" | "danger" | "info" | "text") 按钮类型</div>
    <div>form:String("dialog" | "drawer" | "page" | "tab") 必选 打开方式</div>
    <div>routerPath:String 跳转链接（仅page和tab适用）</div>
    <div>drawerDirection:String("ltr" | "rtl" | "ttb" | "btt") 抽屉打开方向（仅drawer适用）</div>
